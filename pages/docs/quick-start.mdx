...
我们先通过一系列简单的例子，来对 [Transaction DSL]{.title-ref}
有一个直观的认识。

如图所示，站在 [US]{.title-ref} 的角度，一个完整的事务操作，共分为 5
个步骤:

> 1.  从 [Other System 1]{.title-ref} 收到一条请求消息;
> 2.  对 [Other System 2]{.title-ref} 进行了一个函数调用;
> 3.  对 [Other System 3]{.title-ref}
>     发送了一条异步消息，并等待其应答消息;
> 4.  对 [Other System 4]{.title-ref}
>     发送了一条异步消息，并等待其应答消息;
> 5.  给 [Other System 1]{.title-ref} 发送了一条应答消息。

![image](images/ch-2/seq-1.png){.align-center}

## 定义事务

使用 [Transaction DSL]{.title-ref} ，我们将上面序列图中的过程定义为:
...
Action]{.title-ref} )和 **异步操作** ( [Asynchronous Action]{.title-ref}
)。

任何无需进一步等待后续消息的 [Action]{.title-ref} ，都称作 **同步操作**
...
现在我们有了
[Transaction]{.title-ref}，有了基本操作，一个事务就完整了。由于
[Transaction]{.title-ref} 是一个事件驱动的组件。它的基本接口定义如下:
...
![image](images/ch-2/timer-seq.png){.align-center}
...
。其定义的方式是 [C++]{.title-ref} 的常量。 为什么这里可以使用
[C++]{.title-ref} 的语法?

是的，\`Transaction DSL\` 本身是就是 [C++]{.title-ref}
的代码，它可以被任何成熟的 [C++]{.title-ref} 编译器编译。 它是
[C++]{.title-ref} 代码这个事实，让它可以在需要时，使用任何
[C++]{.title-ref} 的元素。

### 母语法

它采取的语法形式非常接近于 [Lisp]{.title-ref} ，除了两点较大的差别:

1.  [Lisp]{.title-ref} 将一个 [list]{.title-ref} 的名字放在括号内;
2.  [Lisp]{.title-ref} 一个 [list]{.title-ref}
    内的各个元素之间无需逗号分割。

如果你以前没有接触过 [Lisp]{.title-ref} 语言，可通过下面一段
[Lisp]{.title-ref} 的例子代码获取直观的印象。
...
无论你是否喜欢 [Lisp]{.title-ref}
语言的语法形式，但对于我们的问题，这已经是我所能找 到的
[C++]{.title-ref} 元编程的最好表现形式。
但两者相似的地方不仅仅是语法形式，更重要的在于其背后的 **元模型** 。对于
[Lisp]{.title-ref} 语言而言，一切都是 [List]{.title-ref} ，
所以，它本身就在表述一颗 **语法树** 。

而对于 [Transaction DSL]{.title-ref} 而言，一切都是 [Action]{.title-ref}
， 它本身就在表述一颗 [Action Tree]{.title-ref}
，在这颗树上，每一个节点所代表的子树都是一个 [Action]{.title-ref} 。
而在叶子节点，就是用户所定义的 **同步** 或 **异步** 的
[Action]{.title-ref} (操作)。

### Timer ID

你已经看到，我们并没有在 [Transaction]{.title-ref} 中直接写 `200ms` 或
`250ms` ，而是使用了 [Timer ID]{.title-ref} 。

将时间约束的数值直接写在 [Transaction]{.title-ref}
...
另外，无论 [Transaction DSL]{.title-ref}
被移植到什么平台上，都必须要满足:

-   在 [Timer]{.title-ref} 过期时，必须发送一条消息出来;
-   [Timer]{.title-ref} 的实现必须能够将 `Timer ID` 和
    [Timer]{.title-ref} 过期消息之间建立起一一映射关系。
    而这一切，在我所见过的各种 [Timer]{.title-ref} 机制中，都不难实现。

### 同步操作的时间约束

虽然你可以在一段 [Transaction DSL]{.title-ref}
...
现在，我们可以使用 [Transaction DSL]{.title-ref}
来对我们最初的过程进行描述:
...
在本章中，我们初步了解了如何使用 [Transaction DSL]{.title-ref}
来定义一个异步过程。 即，实现 **原子操作** ：分为 **同步** 和 **异步**
两种。 然后通过 [Transaction DSL]{.title-ref} 来描述这些 **基本操作**
的执行方式。
...
:   [Transaction DSL]{.title-ref} 的描述，和 [UML]{.title-ref}
...
:   没有复杂的事无巨细的状态机，没有重复繁琐的定时器操作，没有亲力亲为的并发过程控制------开发这些代码是一个无趣的，
    ...
    [UML]{.title-ref} 序列图自动转化为 [Transaction DSL]{.title-ref}
    ，这样就可以连这些代码也不用写了。
...
:   [Transaction DSL]{.title-ref} 本身就是 [C++]{.title-ref}
    代码，这就意味着它可以和 [C++]{.title-ref} 的其它代码元
    素自由地结合使用，与其它 [C++]{.title-ref} 代码进行无缝配合。

图灵完备:

:   随着后续章节的介绍，你就可以发现， [Transaction DSL]{.title-ref}
